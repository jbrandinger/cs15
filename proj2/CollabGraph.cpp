...
vector<Artist> CollabGraph::get_vertex_neighbors(const Artist& artist) const 
{
    ...
    Vertex *v = graph.at(artist.get_name());
    
    int size = v->neighbors.size();
    for (int i = 0; i < size; i++) {
        neighbors.push_back(v->neighbors[i].neighbor->artist);
    }
                                       ...
                                       const Artist& dest) const 
{
    ...
    while (path.top() != source) {
        if (!is_vertex(get_predecessor(path.top()))) {
            int size = path.size();
            for (int i = 0; i < size; i++) {
                path.pop();
            }
            ...
            path.push(get_predecessor(path.top()));
        }
    }
    return path;
}
